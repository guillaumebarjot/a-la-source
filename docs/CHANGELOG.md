...
- **Images cachées en local (anti link-rot, déployé en prod)** : `server/src/scripts/completion/cache-images-local.ts` télécharge les og:image dans `image-cache/source-N` et réécrit `image_url` en `/images/source-N` ; il re-dérive l'og:image pour les chemins locaux qui étaient cassés (fichiers manquants). Résultat en prod : **132 images servies en local** (124 http rapatriées + 8 chemins locaux dont 7 re-remplis), 2 échecs gardent leur URL externe, 28 sources restent sans image (PDF, vidéos, pages sans og:image, liens morts : aucune récupération automatique possible, repli sur l'initiale).
- **Prod en mode WAL** : la base n'étant plus sur OneDrive (contrainte qui imposait le mode DELETE), passage en **WAL** (`synchronous=NORMAL`, `busy_timeout`) : lecteurs concurrents pendant une écriture, tient sans peine plusieurs dizaines d'utilisateurs.
- **Ménage activités fantômes** : suppression de l'atelier vide #1 et du dossier de test #8 (audit 21/06). Les 2 paires de doublons signalées par l'audit étaient déjà résolues (aucune URL en double ne subsiste).
